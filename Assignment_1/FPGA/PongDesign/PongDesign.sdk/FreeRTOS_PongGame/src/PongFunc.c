...
void hitDetect()
{
	...
	//--------------------------
	if(BallY == 0 || BallY == 7)    //check if hit top or bottom
		BallMoveY = BallMoveY * -1;

	if(BallX == 7)	//check if p1 has scored
	{
		scorePlayer1 ++;
		xil_printf("P1 scored\r\n");
		startPositions();
	}
	if(BallX == 0)	//check if p2 has scored
	{
		scorePlayer2 ++;
		xil_printf("P2 scored\r\n");
		startPositions();
	}
}
